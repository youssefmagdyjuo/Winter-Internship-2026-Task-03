        ...
        const productsWithSellerName = await Promise.all(
            products.map(async (product) => {
                const sellerRes = await axios.get(
                    `${import.meta.env.VITE_API_URL}/v1/api/users/${product.sellerId}`
                );
                const categoryRes = await axios.get(
                    `${import.meta.env.VITE_API_URL}/v1/api/categories/${product.categoryId}`
                );
                return {
                    ...product,
...
export const fetchApprovedProductsData = async () => {
    try {
        //fetch products
        const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/v1/api/products/approved`
        );
        const products = response.data.data;
        //fetch seller name for each product
        const productsWithSellerName = await Promise.all(
...
export const fetchCategories = async () => {
    try {
        //fetch categories
        const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/v1/api/categories`
        );
